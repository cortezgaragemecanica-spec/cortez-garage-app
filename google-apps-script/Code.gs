...
function stockList_(){const sheet=stockSheet_(),last=sheet.getLastRow();if(last<2)return[];return sheet.getRange(2,1,last-1,8).getValues().filter(row=>clean_(row[1])).map(row=>({id:clean_(row[0]),description:clean_(row[1]),application:clean_(row[2]),quantity:Number(row[3])||0,value:Number(row[4])||0,photo:clean_(row[5]),invoicePhoto:clean_(row[6]),updatedAt:row[7]instanceof Date?Utilities.formatDate(row[7],Session.getScriptTimeZone(),'dd/MM/yyyy HH:mm'):clean_(row[7])}))}
...
function stockSave_(items){const sheet=stockSheet_(),current=stockList_(),index=new Map(current.map((item,i)=>[`${item.description}|${item.application}`.toLowerCase(),i+2]));items.forEach(item=>{const description=clean_(item.description),application=clean_(item.application);if(!description)return;const key=`${description}|${application}`.toLowerCase(),found=index.get(key),existing=current.find(value=>`${value.description}|${value.application}`.toLowerCase()===key),id=existing?.id||Utilities.getUuid(),photo=stockMedia_(item.photo||existing?.photo||'',`${id}-peca`),invoice=stockMedia_(item.invoicePhoto||'',`${id}-nota`),row=[id,description,application,Number(existing?.quantity||0)+Number(item.quantity||0),Number(item.value||0),photo,invoice||existing?.invoicePhoto||'',new Date()];if(found)sheet.getRange(found,1,1,8).setValues([row]);else{sheet.appendRow(row);index.set(key,sheet.getLastRow())}})}
