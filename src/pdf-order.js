...
const blobBase64=blob=>new Promise((resolve,reject)=>{const reader=new FileReader;reader.onload=()=>resolve(String(reader.result).split(',')[1]);reader.onerror=()=>reject(reader.error||new Error('Falha ao preparar o PDF'));reader.readAsDataURL(blob)});
...
async function generatePdf(button){
  ...
  try{const fileName=`Cortez-Garage-OS-${plain(order.number)}-${plain(order.vehicle?.plate||'sem-placa')}.pdf`,blob=pdfBlob(reportLines(order));if(window.CortezAndroid?.savePdf){window.CortezAndroid.savePdf(await blobBase64(blob),fileName)}else{const file=new File([blob],fileName,{type:'application/pdf'});if(navigator.share&&navigator.canShare?.({files:[file]})){try{await navigator.share({title:`OS #${order.number}`,text:`Ordem de serviço #${order.number} - Cortez Garage`,files:[file]})}catch(error){if(error.name==='AbortError')return;downloadPdf(blob,fileName)}}else downloadPdf(blob,fileName)}}catch(error){if(error.name!=='AbortError')alert(`Não foi possível gerar o PDF.\n\n${error.message}`)}finally{button.disabled=false;button.textContent=original}
}
