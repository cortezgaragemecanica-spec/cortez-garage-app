...
const commissionRate=item=>Number.isFinite(Number(item?.commissionRate))?Math.min(1,Math.max(0,Number(item.commissionRate))):.5;
...
export function appendOrderItemsOnly(remoteBudget,candidateBudget,discount){
  ...
  if(oldParts.some((item,index)=>!unchangedPart(item,newParts[index]))||oldServices.some((item,index)=>!unchanged(item,newServices[index],serviceFields,['value'])||Math.abs(commissionRate(item)-commissionRate(newServices[index]))>.0001))throw new Error('Esta permissão não permite alterar itens já cadastrados.');
