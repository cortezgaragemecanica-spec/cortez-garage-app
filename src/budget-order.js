...
function wire(order,db){
  ...
  const collect=()=>{const parts=collectRows('#partsRows','part'),services=collectRows('#servicesRows','service'),partsTotal=parts.reduce((sum,item)=>sum+item.quantity*item.value,0),servicesTotal=services.reduce((sum,item)=>sum+item.value,0),discount=Number(order.discount||0);order.budget={parts,services,partsTotal,servicesTotal,total:partsTotal+servicesTotal-discount,paymentTerms:document.querySelector('#budgetPayment').value.trim(),warrantyTerms:document.querySelector('#warrantyTerms').value.trim(),checklist:[...document.querySelectorAll('[data-check]')].map(input=>({label:serviceChecklist[Number(input.dataset.check)],ok:input.checked}))};order.partsValue=partsTotal;order.labor=servicesTotal;order.total=order.budget.total;order.updatedAt=new Date().toISOString();localStorage.setItem(DB_KEY,JSON.stringify(db))};
  const persist=async(manual=false)=>{collect();const state=document.querySelector('#budgetState'),button=document.querySelector('#saveBudget');if(saving){pending=true;return}saving=true;if(button&&manual)button.disabled=true;if(state)state.textContent='Salvando no banco…';try{const synced=await saveSupabase(db);localStorage.setItem(DB_KEY,JSON.stringify(synced));document.dispatchEvent(new CustomEvent('cortez:database-updated',{detail:synced}));if(state)state.textContent='Salvo no banco'}catch(error){if(state)state.textContent='Banco pendente';if(manual)alert(error.message)}finally{saving=false;if(button)button.disabled=false;if(pending){pending=false;persist(false)}}};
