...
function row(type,item={},index=0){
  if(type==='part')return`<tr><td class="part-description-cell"><textarea class="part-description" data-field="description" rows="2" placeholder="Escreva o nome completo da peça utilizada">${escapeHtml(item.description)}</textarea></td><td><input data-field="quantity" type="number" min="0" step="1" value="${Number(item.quantity||1)}"></td><td><input data-field="value" type="text" inputmode="decimal" value="${moneyInput(item.value)}"></td><td class="line-total">${money(Number(item.quantity||1)*Number(item.value||0))}</td><td><button type="button" class="remove-line" aria-label="Remover peça">×</button></td></tr>`;
