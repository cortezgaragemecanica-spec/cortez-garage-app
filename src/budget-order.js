...
function wire(order,db){
  ...
  document.querySelector('#servicePhotos').onchange=event=>{[...event.target.files].forEach(file=>{const reader=new FileReader();reader.onload=()=>{photos.push(reader.result);order.budget={...(order.budget||{}),photos};localStorage.setItem(DB_KEY,JSON.stringify(db));mount(true)};reader.readAsDataURL(file)})};
