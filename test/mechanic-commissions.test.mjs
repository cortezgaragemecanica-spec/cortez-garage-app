...
test('atalho de comissões dos mecânicos fica ao lado do Financeiro',async()=>{
  ...
  assert.match(worker,/cortez-garage-v192/);
});
...
test('mecânico solicita orçamento de serviços e proprietário vê a tabela',async()=>{
  ...
  assert.match(index,/service-quote-requests\.js\?v=20260915-1/);
  assert.match(worker,/service-quote-requests\.js\?v=20260915-1/);
});
