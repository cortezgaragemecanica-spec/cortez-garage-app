...
test('botão de solicitar orçamento de serviços aparece em azul',async()=>{
  ...
  assert.match(index,/style\.css\?v=20260923-1/);
  assert.match(worker,/style\.css\?v=20260923-1/);
  assert.match(worker,/cortez-garage-v224/);
});
...
test('painel soma comissões de sócios e mecânicos e detalha em dois níveis',async()=>{
  ...
  assert.match(admin,/Saldo separado por \$\{partners\?'sócio':'mecânico'\}/);
  ...
  assert.match(style,/\.commission-group-open/);
  assert.match(index,/style\.css\?v=20260923-1/);
  assert.match(index,/admin\.js\?v=20260923-3/);
  assert.match(worker,/cortez-garage-v224/);
});
