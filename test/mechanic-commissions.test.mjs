...
test('atalho de comissões dos mecânicos fica ao lado do Financeiro',async()=>{
  ...
  assert.match(worker,/cortez-garage-v194/);
});
...
test('mecânico solicita orçamento de serviços e proprietário vê a tabela',async()=>{
  ...
  assert.match(index,/service-quote-requests\.js\?v=20260915-2/);
  assert.match(worker,/service-quote-requests\.js\?v=20260915-2/);
});
...
test('botão de solicitar orçamento de serviços aparece em azul',async()=>{
  ...
  assert.match(index,/style\.css\?v=20260915-4/);
  assert.match(worker,/style\.css\?v=20260915-4/);
  assert.match(worker,/cortez-garage-v194/);
});
