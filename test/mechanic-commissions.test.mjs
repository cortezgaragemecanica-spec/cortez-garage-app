...
test('atalho de comissões dos mecânicos fica ao lado do Financeiro',async()=>{
  ...
  assert.match(worker,/cortez-garage-v209/);
});
...
test('ordens entregues são separadas por semana',async()=>{
  ...
  assert.match(index,/main\.js\?v=20260915-4/);
});
...
test('ações do orçamento ficam juntas e o salvamento manual é removido',async()=>{
  ...
  assert.match(index,/budget-order\.js\?v=20260917-1/);
  assert.match(index,/order-workflow\.js\?v=20260916-2/);
  assert.match(worker,/cortez-garage-v209/);
});
...
test('peça de orçamento ignora o estoque e mantém todos os campos comerciais',async()=>{
  ...
  assert.match(index,/budget-order\.js\?v=20260917-1/);
  assert.match(worker,/budget-order\.js\?v=20260917-1/);
});
...
test('contas a pagar possuem pesquisa, agrupamento, edição e pagamento parcial',async()=>{
  ...
  assert.match(index,/admin\.js\?v=20260917-2/);
  assert.match(worker,/cortez-garage-v209/);
});
...
test('painel inicial mostra O.S. abertas modificadas sem regravar as demais',async()=>{
  ...
  assert.match(index,/main\.js\?v=20260915-4/);
  assert.match(worker,/main\.js\?v=20260915-4/);
});
...
test('mecânico solicita orçamento de serviços e proprietário vê a tabela',async()=>{
  ...
  assert.match(index,/service-quote-requests\.js\?v=20260917-1/);
  assert.match(worker,/service-quote-requests\.js\?v=20260917-1/);
});
...
test('proprietário precifica a solicitação e envia os serviços para a O.S. sem duplicar',async()=>{
  ...
  assert.match(index,/budget-order\.js\?v=20260917-1/);
  assert.match(index,/service-quote-requests\.js\?v=20260917-1/);
  assert.match(worker,/supabase\.js\?v=20260917-1/);
  assert.match(worker,/cortez-garage-v209/);
});
...
test('botão de solicitar orçamento de serviços aparece em azul',async()=>{
  ...
  assert.match(index,/style\.css\?v=20260917-3/);
  assert.match(worker,/style\.css\?v=20260917-3/);
  assert.match(worker,/cortez-garage-v209/);
});
...
test('painel de solicitações de peças fica junto ao de orçamentos de serviços no início',async()=>{
  ...
  assert.match(index,/order-workflow\.js\?v=20260916-2/);
  assert.match(worker,/order-workflow\.js\?v=20260916-2/);
  assert.match(worker,/cortez-garage-v209/);
});
...
test('proprietário altera internamente as comissões dos sócios sem mudar os textos exibidos',async()=>{
  ...
  assert.match(admin,/await savePartnerCommissionRates\(\{Fabiano:fabiano\/100,Marcelino:marcelino\/100\}\)/);
  assert.match(admin,/allocationBase\*partnerRates\.Fabiano/);
  assert.match(admin,/allocationBase\*partnerRates\.Marcelino/);
  assert.match(admin,/PARTNER_DISPLAY_RATES=\{Fabiano:\.25,Marcelino:\.2\}/);
  assert.match(admin,/readPartnerCommissionRates\(\)/);
  assert.match(reports,/Math\.max\(0,profit\)\*partnerRates\.Marcelino/);
  assert.match(reports,/Math\.max\(0,profit\)\*partnerRates\.Fabiano/);
  ...
  assert.match(supabase,/partnerCommissionRates:rates/);
  assert.match(index,/admin\.js\?v=20260917-2/);
  assert.match(index,/reports\.js\?v=20260917-1/);
  assert.match(worker,/supabase\.js\?v=20260917-2/);
  assert.match(worker,/cortez-garage-v209/);
});
